...
新增推送地址兼容时，后端与主、备中转实例必须同步发布；只更新后端不能让旧中转接受新地址。
...
- 仅转发到精确 `fcm.googleapis.com` 和 `jmt17.google.com`（部分浏览器返回的 Google 推送地址），保留原始订阅 URL 与对应 VAPID audience，不跟随重定向；不开放给浏览器调用，不开启请求正文/凭据日志。
