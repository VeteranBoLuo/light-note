<template>
  <CommonContainer title="威胁总览" @backClick="router.push('/securityCenterMobile')">
    <div>
      <ul class="admin-stats security-stats mobile-stats">
      ...
      <div class="security-grid mobile-security-grid">
      ...
      <div v-if="overview.recentEvents?.length" class="security-section" style="margin-top: 12px">
        <h3>近期安全事件</h3>
        <BTable :data="overview.recentEvents" :columns="mobileRecentColumns" :rowKey="'eventId'" />
      </div>
    </div>
  </CommonContainer>
</template>
...
import { typeColumns, trendColumns, topIpColumns, topPathColumns } from './securityShared';
