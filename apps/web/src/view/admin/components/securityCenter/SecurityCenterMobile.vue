<template>
  <CommonContainer :title="t('securityV2.title')" @back-click="router.push('/admin')">
    ...
    <div ref="mobileTop" class="security-mobile-content">
      ...
      <div ref="reviewSection" class="security-mobile-section-title"><strong>{{ t('securityV2.mobile.reviewEvents') }}</strong><span>{{ t('securityV2.mobile.viewAll', { count: summary.pendingReview || 0 }) }}</span></div>
      ...
      <div ref="qualitySection" class="security-mobile-section-title"><strong>{{ t('securityV2.mobile.qualityAlert') }}</strong><span>{{ t('securityV2.mobile.desktop') }}</span></div>
      <article v-for="rule in noisyRules.slice(0, 2)" :key="rule.ruleCode" class="security-mobile-event">
    ...
    <nav class="security-mobile-bottom-nav">
      <BButton :class="{ 'is-active': mobileTab === 'overview' }" @click="scrollTo('overview')">{{ t('securityV2.nav.overview') }}</BButton>
      <BButton :class="{ 'is-active': mobileTab === 'review' }" @click="scrollTo('review')">{{ t('securityV2.nav.review') }}</BButton>
      <BButton :class="{ 'is-active': mobileTab === 'quality' }" @click="scrollTo('quality')">{{ t('securityV2.nav.quality') }}</BButton>
      <BButton @click="router.push('/securityCenter/access-control')">{{ t('securityV2.nav.access') }}</BButton>
    </nav>
...
<script lang="ts" setup>
  ...
  const mobileTab = ref('review');
  const mobileTop = ref<HTMLElement>();
  const reviewSection = ref<HTMLElement>();
  const qualitySection = ref<HTMLElement>();
  ...
  function scrollTo(section: string) { mobileTab.value = section; ({ overview: mobileTop, review: reviewSection, quality: qualitySection } as any)[section].value?.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
