  ...
  <div v-if="downloads.length" class="native-download-progress">
    <div v-for="item in downloads" :key="item.id" class="native-download-item">
      ...
      <div
        ...
        :class="{ 'is-indeterminate': isIndeterminate(item), 'is-failed': item.status === 'failed' }"
        role="progressbar"
        :aria-valuemin="0"
        :aria-valuemax="100"
...
<script setup lang="ts">
  ...
  function isIndeterminate(item: AndroidDownloadProgress) {
    return item.percent < 0 && item.status !== 'failed';
  }

  /*
   * 失败和不确定态都不给内联宽度：内联 style 会压过样式表里的 `.is-failed{width:100%}`
   * 和不确定态的动画，失败时红条会变成 0 宽（等于看不见）。
   */
  function barStyle(item: AndroidDownloadProgress) {
    if (item.status === 'failed' || isIndeterminate(item)) return undefined;
