      ...
      <div class="bsnap-bar">
        ...
        <BSpace v-if="!isGuest">
          ...
          >
            ...
            {{
              generating
                ? $t('bookmarkMg.snapshotGenerating')
                : snap?.content || snap?.summary
                  ? $t('bookmarkMg.snapshotUpdate')
                  : $t('bookmarkMg.snapshotGenerate')
            }}
          </BButton>
        </BSpace>
      </div>
...
<script lang="ts" setup>
  ...
  async function generateArchive() {
    if (!props.bookmarkId || generating.value) return;
    const bookmarkId = props.bookmarkId;
    const isUpdate = Boolean(snap.value?.content || snap.value?.summary);
    generating.value = true;
    try {
      const res = await apiBasePost('/api/bookmark/archive-summary', { id: bookmarkId });
      ...
      if (res?.status === 200 && res.data?.ok) {
        ...
        await loadSnap();
      } else if (res?.status === 200 && res.data?.archiveOk) {
        await loadSnap();
        message.info(res.data?.msg || t('bookmarkMg.snapshotPartial'));
      } else {
  ...
  watch(
    ...
    async ([isVisible, bookmarkId]) => {
      ...
      if (!isVisible || !bookmarkId) {
        autoGeneratedBookmarkId = '';
        generating.value = false;
        return;
      }
      snap.value = null;
      const currentSnap = await loadSnap();
      if (!visible.value || bookmarkId !== props.bookmarkId || isGuest.value) return;
      if ((currentSnap?.content && currentSnap?.summary) || autoGeneratedBookmarkId === bookmarkId) return;
      autoGeneratedBookmarkId = bookmarkId;
