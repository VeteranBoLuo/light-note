<template>
  <div class="growth-tasks" :class="{ 'growth-tasks--compact': compact }">
    ...
    <p class="growth-tasks__remaining">
      {{ remainingCount > 0 ? t('growth.tasksRemaining', { n: remainingCount }) : t('growth.tasksAllDone') }}
    </p>
    ...
    <div v-if="visibleTasks.length" class="growth-tasks__list">
      <article
        ...
        :key="task.taskKey"
        class="growth-task"
        :class="{ 'growth-task--completed': task.completed }"
      >
        ...
        <BButton v-if="!task.completed && !readOnly" class="growth-task__go" size="small" @click="navigateToTask(task)">
          {{ t('growth.tasksGoTo') }}
        </BButton>
        <span v-else-if="task.completed" class="growth-task__completed">{{ t('growth.tasksCompleted') }}</span>
        <span v-else class="growth-task__readonly">{{ t('growth.tasksReadOnly') }}</span>
      </article>
    </div>
...
<script setup lang="ts">
  ...
  import type { GrowthTask, GrowthTasksData } from '@/composables/useGrowth.ts';
  ...
  const visibleTasks = computed(() => {
    const tasks = props.showCompleted ? allTasks.value : pendingTasks.value;
