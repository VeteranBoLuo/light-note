...
describe('NoteTagConfig 移动端高度分配', () => {
  it('只让标签库列表纵向滚动，摘要与底部操作保持固定', () => {
    ...
    expect(source).toMatch(/\.tag-config-shell\.is-sheet\s*\{[\s\S]*?height:\s*100%;[\s\S]*?overflow:\s*hidden;/);
    expect(source).toMatch(/\.tag-config\.mobile\s*\{[\s\S]*?flex:\s*1 1 auto;[\s\S]*?overflow:\s*hidden;/);
    ...
    expect(source).toMatch(/\.tag-config-footer\s*\{[\s\S]*?flex:\s*0 0 auto/);
  });

  it('把移动端已选摘要压缩为数量徽标和横向标签带', () => {
    expect(source).toContain('class="selected-count-badge"');
    expect(source).toMatch(/\.tag-config\.mobile\s*\{[\s\S]*?\.selected-overview\s*\{[\s\S]*?display:\s*none/);
    expect(source).toMatch(/\.tag-config\.mobile\s*\{[\s\S]*?\.chip-list\s*\{[\s\S]*?flex-wrap:\s*nowrap/);
    expect(source).toMatch(/\.tag-config\.mobile\s*\{[\s\S]*?\.chip-list\s*\{[\s\S]*?overflow-x:\s*auto/);
  });

  it('移动渲染基线用实色描边和勾选图标表达绑定状态', () => {
    expect(source).toContain(':src="icon.filterPanel.check"');
    expect(source).toContain('html.light-note-mobile-rendering .tag-config.mobile .tag-row.active');
