...
describe('cloud file empty state layout', () => {
  ...
  it('列表表头与普通行 hover 使用稳定主题色，不再动态混入资源橙色', () => {
    expect(source).toMatch(/\.field-header,\s*\.field-item,\s*\.file-card\s*\{\s*\.workspace-content-surface\(\);/);
    expect(source).toMatch(/\.field-item:hover\s*\{\s*background:\s*var\(--workspace-hover\);\s*\}/);
