...
export function useNotification() {
  ...
  async function refreshUnread() {
    ...
    try {
      ...
      if (res?.status === 200 && res.data) unreadTotal.value = Number(res.data.unreadTotal) || 0;
    } catch {
      /* 忽略,下次轮询再试 */
    }
  }
  ...
  return { unreadTotal, refreshUnread, fetchList, markRead, markAllRead };
}
