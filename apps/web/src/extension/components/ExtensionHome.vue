<template>
  <section class="ln-extension-view ln-extension-home">
    <p class="ln-extension-pin-hint">
      <span aria-hidden="true"><SvgIcon :src="icon.contextMenu.pin" size="16" /></span>
      {{ t('browserExtension.home.pinHint') }}
    </p>
    ...
    <div
      class="ln-extension-session-card"
      :class="{ 'is-authenticated': authenticated, 'is-unauthenticated': !authenticated }"
    >
      ...
      <BButton v-if="authenticated" size="small" @click="emit('logout')">
        {{ t('browserExtension.login.logout') }}
      </BButton>
      <BButton v-else type="primary" @click="emit('login')">
        {{ t('browserExtension.login.action') }}
      </BButton>
    </div>
