...
1. 冷启动、返回键、横竖屏切换和刘海屏 / 底部手势区域。
2. 登录、退出、GitHub OAuth 回调和登录态保持。
3. 新建、编辑、删除和搜索笔记，上传图片与文件。
4. 外部网页、电话、邮件、地图以及被拒绝的未知协议。
5. 断网错误页、恢复网络后重试、SSL 错误阻断和文件下载。
6. 唤起软键盘后输入框不被遮挡，关闭键盘后页面尺寸恢复。
