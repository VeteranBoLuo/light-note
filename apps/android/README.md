...
- Root 灰度账号可把通知中心普通未读同步到 Android 通知栏，并用未读数校准桌面图标数字角标；聊天室只在用户单独开启后提醒回复或提及，且其 channel 明确不参与桌面角标。当前链路依赖 App 进程与网页运行时存活，尚不等同于应用被彻底结束后的厂商推送。
...
当前隐私政策版本为 `2026-08-11`（用户协议仍为 `2026-07-28`）。定稿依据、权限、依赖和第三方服务审计见
