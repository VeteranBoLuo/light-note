...
const GROUPS = {
  ...
  adminResourceStats: ['get_resource_creation_ranking'],
  adminCheckinRanking: ['get_checkin_ranking'],
...
const DOMAIN_INTENT_RULES = [
  ...
  {
    pattern:
      /(?:签到(?:天数)?|连续签到|连签).{0,16}(?:排行|排名|榜)|(?:排行|排名|榜).{0,16}(?:签到|连续签到|连签)|(?:最长|当前).{0,8}(?:连签|签到)/i,
    groups: ['growth', 'adminCheckinRanking'],
  },
  ...
  {
    pattern: /(?:新增|创建).{0,12}(?:资源|内容).{0,12}(?:最多|排行|排名)|谁.{0,12}(?:新增|创建).{0,12}(?:资源|内容)/i,
    groups: ['adminResourceStats'],
  },
