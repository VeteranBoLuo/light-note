...
export default {
  ...
  routing: {
    ...
    requireAny: [
      /(?:保存|存入|上传).{0,20}(?:附件|原文件|图片|文件).{0,20}(?:云空间|文件夹)|(?:附件|原文件|图片|文件).{0,20}(?:保存|存入|上传).{0,20}(?:云空间|文件夹)/iu,
    ],
    preferAny: [
      /(?:附件|原文件|图片|文件).{0,24}(?:云空间|文件夹)|(?:云空间|文件夹).{0,24}(?:附件|原文件|图片|文件)/iu,
    ],
  },
  parameters: {
    type: 'object',
    properties: {
      ...
      folderStrategy: {
        ...
        description: '文件夹处理策略；默认只使用已有文件夹',
      },
    },
