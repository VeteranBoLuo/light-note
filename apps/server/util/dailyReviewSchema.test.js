...
describe('daily review schema', () => {
  it('多实例同时补日期列时只忽略 duplicate-column 竞态', async () => {
    ...
    expect(poolQuery.mock.calls[3][0]).toContain('CREATE TABLE IF NOT EXISTS daily_review_sessions');
    expect(poolQuery.mock.calls[4][0]).toContain('CREATE TABLE IF NOT EXISTS daily_review_items');
    expect(poolQuery.mock.calls[6][0]).toContain('ALTER TABLE daily_review_items ADD COLUMN resource_date');
  });
