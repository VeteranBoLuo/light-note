...
describe('root 真实等级口径', () => {
  ...
  it('已固化的历史 root 按真实 50000 EXP 返回 Lv.15 和 50 万 AI', async () => {
    vi.clearAllMocks();
    mockGrowthRow(50_000, 15);

    const growth = await getGrowth('root-1', { userRole: 'root', calendar: accountCalendar('20260825') });

    expect(growth).toMatchObject({ exp: 50_000, level: 15, aiTokenDaily: 500_000, spaceMb: 20_480 });
  });
  ...
  it('未经迁移的低经验 root 不再被展示层伪造成 Lv.15', async () => {
    ...
    expect(growth).toMatchObject({ exp: 0, level: 1, aiTokenDaily: 100_000, spaceMb: 1024 });
  });
});
