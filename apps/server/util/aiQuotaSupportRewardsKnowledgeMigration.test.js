...
const quotaRebalanceMigrationUrl = new URL(
  '../migrations/20260831_ai_quota_rebalance_knowledge.sql',
  import.meta.url,
);
...
const quotaMigrationUrls = [
  ...
  '../migrations/20260825_ai_quota_and_support_rewards_knowledge.sql',
  '../migrations/20260825_support_donation_store_split_knowledge.sql',
...
describe('AI 额度与赞助赠送公开知识迁移', () => {
  it('最新额度文章覆盖为 5 万游客额度与 20～80 万等级曲线', async () => {
    const source = await readFile(fileURLToPath(quotaRebalanceMigrationUrl), 'utf8');

    expect(source).toContain('START TRANSACTION;');
    expect(source).toContain('COMMIT;');
    expect(source).toContain('data-ln-policy:ai-quota-v2');
    expect(source).toContain('游客按设备每日 5 万 tokens');
    expect(source).toContain('Lv.1～5 分别为 20、22、24、26、28 万');
    expect(source).toContain('Lv.6～9 为 31、34、37、40 万');
    expect(source).toContain('Lv.10～15 为 45、50、56、62、70、80 万 tokens/日');
    expect(source).toContain('知识工坊等真实模型调用共享同一份额度');
    expect(source).not.toContain('Lv.10～15 为 105、120、140、160、180、200 万');
  });

  it('历史额度、积分和赞助同步保留稳定文章标识与不可降级门禁', async () => {
    ...
    expect(source).toContain("LOCATE('data-ln-policy:ai-quota-v'");
  ...
  it('所有历史可重跑额度知识源都不会覆盖带版本标记的新规则', async () => {
