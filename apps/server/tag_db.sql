...
CREATE TABLE `file_preview_artifacts` (
  ...
  `source_object_key` varchar(1024) DEFAULT NULL,
...
CREATE TABLE `note` (
  ...
  KEY `idx_note_admin_created` (`del_flag`(8),`create_time`,`create_by`(64)),
...
CREATE TABLE `tag` (
  ...
  `active_name` varchar(255) GENERATED ALWAYS AS ((case when (`del_flag` = 0) then `name` else NULL end)) STORED,
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `uk_tag_active_user_name` (`user_id`,`active_name`),
...
-- ----------------------------
-- Table structure for community_chat_poll_multi_votes
-- ----------------------------
DROP TABLE IF EXISTS `community_chat_poll_multi_votes`;
CREATE TABLE `community_chat_poll_multi_votes` (
  `message_id` bigint unsigned NOT NULL,
  `user_id` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
  `option_id` bigint unsigned NOT NULL,
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
