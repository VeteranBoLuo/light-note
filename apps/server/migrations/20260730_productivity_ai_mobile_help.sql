...
SET @ai_scope_help_title = '轻笺智域的作用范围、资源排除与依据边界';
SET @ai_scope_help_content = '<h1>轻笺智域的作用范围、资源排除与依据边界</h1><p>发送问题前，输入区会显示本轮可用的笔记、书签、云文件等材料。你可以只在本轮临时排除其中某些资源，也可以把单条书签、笔记或云文件设为长期不参与 AI。</p><h2>排除范围</h2><p>长期排除和本轮排除都会由服务器执行，覆盖个人内容搜索、直接附加的资源上下文和云文件附件；被排除内容不会交给模型。若系统无法确认资源归属或排除状态，本轮会停止读取，而不是忽略设置继续回答。</p><h2>回答边界</h2><p>回答会说明实际使用的资源、覆盖数量、截断或权限边界。没有检索到账号内依据时，轻笺智域会明确说明未找到，不会把通用知识包装成你的账号事实。引用表示答案绑定了具体来源和位置；引用是否充分支持主张仍会通过离线评测和人工抽检持续检查。</p><h2>会话与配额</h2><p>会话侧栏支持搜索、置顶、文件夹和归档筛选。配额默认显示已用百分比和大致可用轮次，Token 明细可展开查看。当前长期记忆关闭，系统不会新增或使用记忆；历史数据导出是独立的数据管理能力。</p>';
