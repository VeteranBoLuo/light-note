-- 2026-07-18 书签地址校验，以及智能识别覆盖预览、停止与超时说明（MySQL 5.7 兼容）
...
SET @bookmark_meta_faq_id = '51c8d5b2-bc16-45c8-b8ad-76c969d32e7e';
SET @bookmark_meta_faq_title = '书签智能识别如何避免覆盖，以及如何停止生成';
SET @bookmark_meta_faq_content = '<h2>识别结果会直接覆盖吗</h2><p>不会。如果当前名称或描述已有内容，轻笺会先展示“当前内容”和“识别结果”的逐字段对比。名称、描述默认可分别选择，你只需勾选想更新的字段后再应用；选择“保留当前内容”则不修改表单。</p><h2>为什么按钮变成了“停止生成”</h2><p>这表示轻笺正在读取网页并生成书签信息。点击“停止生成”会真正取消当前请求，而不只是隐藏加载动画；已经返回但尚未应用的内容也不会写入表单。</p><h2>一直没有结果怎么办</h2><p>系统设有自动超时，等待过久会终止本次识别并给出提示。你可以检查网络和网址后重试，也可以直接手动填写名称、描述和标签，不影响正常保存书签。</p>';

INSERT INTO knowledge_base
  (id, title, content, category, status, type, sort, created_by, updated_by)
SELECT
  @bookmark_meta_faq_id, @bookmark_meta_faq_title, @bookmark_meta_faq_content,
  'FAQ', 'public', 'html', 103, NULL, NULL
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM knowledge_base WHERE title = @bookmark_meta_faq_title)
  AND NOT EXISTS (SELECT 1 FROM knowledge_base WHERE id = @bookmark_meta_faq_id);

UPDATE knowledge_base
SET content = @bookmark_meta_faq_content, category = 'FAQ', status = 'public', type = 'html', sort = 103, updated_by = NULL
WHERE id = @bookmark_meta_faq_id OR title = @bookmark_meta_faq_title;

COMMIT;

SELECT id, title, category, status, type, sort
FROM knowledge_base
WHERE id IN (@bookmark_url_help_id, @bookmark_url_faq_id, @bookmark_meta_faq_id)
   OR title IN (@bookmark_url_help_title, @bookmark_url_faq_title, @bookmark_meta_faq_title);
