-- 公共聊天室提醒闭环：显式提及关系、四级提醒范围与站内通知幂等投递。
...
-- 旧值 none 的语义收敛为“仅官方/管理员”，新策略使用 official / mentions_only / mentions / all。
UPDATE `community_chat_user_settings`
   SET `default_room_level` = 'official'
 WHERE `default_room_level` NOT IN ('official', 'mentions_only', 'mentions', 'all');
