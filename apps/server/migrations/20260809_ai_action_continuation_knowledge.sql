...
SET @ai_action_continuation_help_content = '<h2>确认卡与后续回答</h2><p>轻笺智域执行创建笔记、书签、标签、待办或其他写操作前，仍会先展示确认卡。只有你点击确认并收到服务端成功回执后，助手才可能在原卡片下方继续回答最初的问题。</p><h2>不会伪造用户消息</h2><p>自动续答是受会话和账号绑定的内部协议事件，不会替你发送“继续回答”之类的用户消息，也不会把内部控制文案写入本地或云端会话。后续回答只读取服务端保存的原问题、必要的查询摘要和成功回执，不会重新进入工具规划，也不会再次创建或修改资源。</p><h2>可信结果</h2><p>卡片中的操作结果始终以确认接口返回的权威回执为准；即使后续 AI 回答生成失败，已经成功的操作也不会被改成失败或自动回滚。</p><h2>什么时候不会继续</h2><ul><li>取消、返回编辑、执行失败或写入结果仍在核验时不会调用 AI 续答。</li><li>同一回答中同时存在多张未决卡片时，当前版本会让每张卡片独立结算，不猜测执行顺序，也不会自动续答。</li><li>保存附件等结构化快捷操作保持原有确定性结果，不会为每次点击额外调用 AI。</li></ul><p>如果自动续答已经过期或暂时失败，可以直接在当前会话继续提问；请不要为了获得回答而重复点击已经成功的创建操作。</p>';
