...
try {
  ...
  const [columns] = await pool.query(
    "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND ((TABLE_NAME='file_preview_artifacts' AND COLUMN_NAME IN ('source_revision','image_width','image_height','preview_metadata_json')) OR (TABLE_NAME='file_preview_jobs' AND COLUMN_NAME='output_keys_json') OR (TABLE_NAME='image_assets' AND COLUMN_NAME IN ('reconciled','delete_started_at')))",
  );
  const ok = runtime.ready && videoRuntime.ready && tables.length === 4 && columns.length === 7;
  console.log('[video-preview-runtime] runtime=%s', videoRuntime.ready ? 'ready' : 'unavailable');
  console.log(
    '[image-preview-runtime] runtime=%s schema=%s',
    runtime.ready ? 'ready' : 'unavailable',
    tables.length === 4 && columns.length === 7 ? 'ready' : 'incomplete',
  );
