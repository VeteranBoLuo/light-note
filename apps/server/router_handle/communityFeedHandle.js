...
export function handle(service, extra = () => ({})) {
  return async (req, res) => {
    ...
    try {
      return res.send(
        resultData(
          await service({ user: req.user, input: req.method === 'GET' ? req.query : req.body, ...extra(req) }),
        ),
      );
    } catch (error) {
...
export const feedCapabilities = handle(async () => ({
  ...(await capabilities()),
...
export const topics = handle(topicService.listTopics),
  listPosts = handle(posts.listPosts),
...
export const postDetail = handle(posts.postDetail, (req) => ({ id: req.params.id }));
...
export const profileOptions = handle(profiles.profileOptions),
  ...
  relationList = handle(profiles.relationList),
  publicProfile = handle(profiles.publicProfile, (req) => ({ id: req.params.id }));
...
export async function profileAvatar(req, res) {
  ...
  try {
    await access(pool, req.user);
    const row = req.params.postId
      ? { id: (await loadPost(pool, req.params.postId, req.user)).author_id }
      : await profiles.publicProfileRow(pool, req.params.id, req.user?.id || '');
...
export async function image(req, res) {
  ...
  if (req.adminContext) return res.sendStatus(403);
  try {
    const data = await images.readImage({ user: req.user, id: req.params.id, input: req.query });
...
export const resource = handle(resources.readResource, (req) => ({ id: req.params.id }));
...
export const topicDetail = handle(topicService.topicDetail, (req) => ({ slug: req.params.slug }));
