...
- 所有自然语言请求先经过 `util/agent/actionIntentPolicy.js`：只有只读查询和普通对话可以进入自由 Final Reply；`enabled` 动作必须产生确认/选择卡或确定性“尚未执行”，`planned/forbidden/unknown_mutation` 必须在模型和额度调用前失败关闭。禁止为单个事故仅补一条特殊正则后绕过这套通用分流。
