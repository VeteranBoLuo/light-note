...
- 单个已校验书签上的明确“生成笔记”是通用 Semantic Planner 的确定性例外：必须先用书签快照，快照不足时仅读取该书签当前 URL 一次，再用强制函数协议产出完整 Markdown 草稿。不得把全部业务工具暴露给该草稿模型，也不得因通用 Planner 漏调工具而退化为普通文本回答。网页正文和旧草稿均是不可信数据，只有当前用户指令可作为改写要求。
...
- 待确认的 `create_note` 草稿遇到“写长一点/详细一点/重写”等明确改写指令时，客户端只发送旧 `confirmationId + token`，旧草稿必须从服务端确认存储中读取。新草稿签发必须原子替换旧令牌，并发送 `tool_confirmation_replaced` 让前端将旧卡结算为已替换；旧卡随后不得再确认或取消。
...
- “重复任务”表示完成当前实例后生成下一实例；“周期提醒”表示同一实例按间隔重复通知。两者必须分别保存、分别展示，不能从一个开关推断另一个。
- 完成重复任务、生成下一实例、清单重置和提醒平移必须在同一事务内完成；同一系列 + 实例时间使用唯一约束防止重复生成。
