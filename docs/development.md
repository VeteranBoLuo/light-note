...
- **⚠️ 状态(2026-07-22):长期记忆已全局关闭**(前端 `memoryMode:'off'` + 后端 `AI_MEMORY_ENABLED=false` 硬开关;账本入口/影响卡片已移除)。以下记忆相关约束为**原设计**,当前不生效,重做记忆前保留作参考。详见 `docs/fix/codex-review-fix.md` P1-2。
