...
- Node.js 版本以根目录 `package.json` 的 `engines` 为准；后端部署的最低版本见[发布规范](./release-acceptance.md)。
- pnpm 使用根目录 `package.json` 的 `packageManager` 锁定版本，不以宽泛最低版本替代。
- 数据库兼容基线为 MySQL 5.7，不默认使用 MySQL 8 专属能力。
...
Vite 开发服务器的 REST、上传写入、Worker 和实时代理固定指向本机后端；`VITE_ENV` 缺省或为 `local` 时允许启动，其他值失败关闭。唯一远程例外是公开静态目录 `/uploads`：本地开发与本地预览仅允许把 GET/HEAD 只读代理到生产站点，转发前必须移除 Cookie、Authorization、会话、管理员上下文和设备标识等凭据，响应也不得向 localhost 写入 Cookie；其他方法直接拒绝。服务端的早期与全局业务限流均不统计 `/uploads` 的 GET/HEAD，但写方法、API 上传入口和相似前缀仍正常限流。生产构建部署后继续使用站点同源 API 与静态资源，不允许用本地 Vite 反向代理生产业务服务，以免调试请求和在线状态进入生产运行时。
...
5. 用户可见 UI 列出受影响状态与端型，局部改动可用简短清单，复杂交互再建立矩阵；覆盖浅深主题及适用的移动渲染基线。
6. 判断是否触发下述[页面原型确认门禁](#页面原型确认门禁)；触发时先完成原型与用户确认，不直接编写业务页面。

采用满足完整约束的最简单方案。局部问题优先局部修复；已有稳定职责或多个实际调用方需要同一行为时再抽取共享能力，不为假设中的未来场景新增配置层、通用框架或页面外壳。需要扩大协议、迁移数据或取得新授权时先说明；阶段性方案必须明确遗留风险和回退边界。

### 页面原型确认门禁

- **必须先出图：** 新功能或新模块涉及新页面、主要工作区，以及既有页面较大范围的布局、信息层级、导航或核心交互重构。按用户看到的变化判断，不以代码行数或是否拆成多个小任务规避门禁。
- **无需强制出图：** 单个组件的颜色、间距、圆角等局部样式调整，文案、图标替换和不改变页面结构或核心操作流程的小修复；纯后端及不可见逻辑改动也不适用。此类改动仍执行原有开发与视觉验收要求。
- **先了解真实功能：** 设计前读取相关页面、组件、数据协议和操作链路，核对现有状态与相邻页面。新增功能以当前需求及已确认能力边界为依据；示例数据与拟议交互须标明，不能凭截图或组件名称猜功能，也不能为了画面丰满虚构字段、统计或操作。
- **先原型，后实现：** 按[页面原型设计标准](./design.md#页面原型设计标准)展示可评审的高保真原型图，说明主要布局与交互取舍，等待用户明确确认具体版本后，才开始对应页面的业务实现。文字方案、低保真线框或先写好页面再截屏不能替代此步骤。未通过就继续调整原型；出图工具不可用时说明阻塞，不直接跳过门禁。
- **确认边界：** “帮我开发”“优化得高级好看”等一般开发授权不等于原型确认；已有明确确认在当前任务中继续有效，不重复索要。用户只授权设计时，确认原型不自动扩大为开发授权。确认前可继续代码阅读与需求梳理。
- **按确认版本落地：** 在任务中保留已确认原型的图片或可访问路径及关键交互约定，按其构图、层级、组件比例与操作方式实现。响应式细化、真实文案换行等适配可自行处理；若需显著改变已确认布局、功能入口或交互，先更新原型并再次确认受影响部分。
- **验收对照：** 在真实浏览器中对照确认原型检查视觉与操作，并完成受影响端型、主题、密度和状态的既有门禁。原型图片不证明动效、响应式或功能已实现；生成图片中的文字、图标或控件瑕疵应按真实文案和共享组件修正，不照搬错误。
...
- 组件专属样式默认放在对应 Vue 文件的 `<style scoped>` 中，可使用 `lang="less"`；不以缩短 Vue 文件或接入界面密度为由单独拆成 CSS/Less 文件。组件职责过多时优先拆分组件，让模板、逻辑与专属样式一起归属。
- 跨组件样式只提取实际复用的部分；主题变量、重置、共享 mixin 和集中维护的第三方适配属于独立样式的合理用途。判断共享看实际使用者与职责，不能因为文件被命名为“公共”或由父页面导入，就把子组件的专属样式全部集中进去。
- 共享样式文件不混入单个页面的布局和组件私有状态。模块级共享选择器使用明确前缀；全局基础样式统一从既有全局入口接入，不在业务页面重复维护。确需单独维护的专属样式应有具体工程原因，并在引用处说明。
- 调整样式归属时检查父子组件、第三方内部节点、Teleport 浮层、选择器优先级与加载顺序；必要时使用局部 `:deep()` 或有明确边界的全局规则，不能机械添加 `scoped` 导致样式失效。外置与内联样式遵守同一套主题、密度和跨端验收规则。
...
- 持续增长的列表必须由服务端分页/游标限制取数；前端按实际节点规模、行复杂度和性能证据决定是否虚拟化。有限摘要与小型分页列表不强制虚拟化，虚拟化也不能代替慢查询治理。
- 连续浏览列表优先在滚动接近末尾时自动续页，不用“加载更多”按钮作为正常续页入口；确需定位页码的分页视图保留分页。追加请求须防并发，沿用已应用的筛选快照；失败保留已加载内容、停止自动重试并提供明确重试入口。
...
- 新增或修改七个主要模块的工作区表面，必须复用共享 Less 表面样式，按[工作区配色](./design.md#工作区配色)选择语义。禁止使用菜单、按钮或输入框背景充当目录／画布，禁止业务页面维护独立浅深区域色值。
- 背景修改同时检查父容器、内部内容、移动覆盖与两套主题；不得通过增加 `!important` 对抗旧背景，应清理旧声明。共享工作区表面变化运行 `pnpm --filter web check:workspace-surfaces`（需要本地 Web/API 与浏览器）；局部操作样式按受影响页面验收，不能用源码字符串测试替代。
...
5. 检查是否引入重复状态、全量请求、轮询、监听、缓存或第二事实源；UI 改动同时按[样式归属与组织](#样式归属与组织)核对专属样式位置、共享文件的实际使用者和作用域。
...
用户可见 UI 按实际影响选择验收范围；共享基础组件、主题与布局规则变动需扩大到调用方，局部修改不机械扩展为全站重验。以下维度按适用项覆盖：
...
本地生产链路预览、Schema/Worker 门禁、分支处理、上线授权和发布后检查统一见 [本地预览与上线](./release-acceptance.md)。提交、推送、远程检查与部署分别按当前任务的对应授权执行，不从普通开发请求推导。
