...
8. **本机服务器管理 Agent：** `apps/host-agent` 必须作为独立 systemd unit 部署，先按 `apps/host-agent/README.md` 完成专用账户、Unix Socket、PM2 访问模式、root 所有 helper 与精确 sudoers 的预检。非 root PM2 使用同账户 `direct`；遗留 root PM2 只能使用 `helper` 的精确命令桥接，不得共享 `/root/.pm2`、加入 root 组或开放 `pm2 *`。它不得加载 `apps/server/.env`，不得开放 TCP 端口，不得把 Socket 放宽到 `0666`。发布顺序为共享协议 → Host Agent → Express → Web；协议不一致时页面必须失败关闭。发布后通过 `/v1/health` Socket 请求、Agent unit 日志和 Root 服务器管理页共同验收。
