...
- Root Android App 灰度原生通知使用独立同源 `/realtime/notifications`，只从 sid 恢复并定期复核 Root 身份；客户端不得提交账号，服务端事件不得携带通知正文，只能触发受权 REST 权威重读。普通通知的桌面图标数字只取排除 `community_chat` 后的未读总数；聊天室回复/@ 必须使用 `showBadge=false` 的独立 Android channel，默认关闭且永不参与桌面角标。基线同步、轮询补偿、跨端已读只静默校准，只有检测到新的普通未读或用户已开启的聊天定向记录才允许响铃。账号退出/切换和总开关关闭必须清空原生通知；没有厂商 Push/FCM/HMS 时不得承诺进程被彻底结束后的实时通知。
