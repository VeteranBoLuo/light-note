...
- 待确认的 `create_note` 草稿遇到“写长一点/详细一点/重写”等明确改写指令时，客户端只发送旧 `confirmationId + token`。旧草稿以及“原始提问 + 已校验资源稳定引用 + 附件来源 ID”必须从服务端确认存储读取；资源正文不做客户端回传，也不在确认卡公开数据中泄漏。服务端应重新校验归属并重取最新材料后改写。新草稿签发必须原子替换旧令牌，并发送 `tool_confirmation_replaced` 让前端将旧卡结算为已替换；旧卡随后不得再确认或取消。
