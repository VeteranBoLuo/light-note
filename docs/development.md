...
- Root 投票和逐消息已读必须分别受 `COMMUNITY_CHAT_POLLS_ENABLED`、`COMMUNITY_CHAT_READ_RECEIPTS_ENABLED` 显式子开关控制，并继续依赖消息总开关；关开关只停止新建/投票/记录，不删除历史投票或回执。投票问题属于消息正文，选项、截止时间与一账号一票的当前选择属于子表；创建仍复用消息事务、全负载幂等、提及和通知链路，但既有文本/表情消息指纹版本不得随新消息类型整体升级，避免滚动发布时同一 `clientRequestId` 被误判冲突。同一投票草稿的网络重试必须复用客户端请求 ID，服务端必须先识别既有投票再校验相对当前时间的截止窗口，不能因时间流逝拒绝已经成功的幂等重放。截止时间输入必须带 `Z` 或数值偏移并转换为 UTC，SQL 判定只使用 `UTC_TIMESTAMP(3)`；禁止把会话时区下的 `DATETIME` 与带 `_utc` 语义的列直接比较。逐消息回执在发送 Root 消息时固化全局子开关，只记录登录用户对当前可见 active 消息的首次阅读，使用 `(message_id, user_id)` 幂等保存首次可见时间；作者本人、游客、个人删除、屏蔽和隐藏消息不计入，响应仅给 Root 聚合数，不返回名单。气泡正文只有在页面可见、浏览器窗口聚焦且前台连续可见至少 800ms 后，才进入每批最多 30 条的写入队列；滚动、布局变化、失焦、切后台和恢复前台都必须重置连续计时，弱网失败使用静默退避重试，超过一批的同屏回执必须继续排空；房间 `community_chat_reads` 仍只表示未读游标，禁止拿它推导逐条阅读。回执资格查询禁止用互斥锁串行化消息行或 Root 账号，但必须用 MySQL 5.7 共享锁与账号注销建立先后顺序；投票事件仅定向投票作者，前端对密集事件合并在途刷新并保证结束后补一次。
