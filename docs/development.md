...
- 最终回答必须经过可观测质量门禁：工具/证据/动作相关的事实回答最高温度为 `0.6`，并在服务端完整缓冲通过检查后才发送；普通流式回答必须在公开每批安全增量前检查可验证退化信号，命中后主动取消 Provider 流；供应商截断、协议/结束标记泄漏、超长无断句和重复退化只允许进行一次禁用工具的低温短答重试，且异常前缀与重试正文不得拼接为用户可见终态；重试仍失败时返回稳定降级文案。质量重试不得补做业务工具、不得绕过引用审计或“无回执，不成功”约束。
