...
- Markdown 下划线统一保存为无属性的 `<u>文字</u>`，不新增 `++文字++` 等私有方言。编辑器工具栏、`Ctrl/⌘ + U`、重复上一步、HTML/Markdown 转换、转换风险分析、站内预览和离线导出必须共用这一语义；带属性的 `u` 仍按未知 raw HTML 提示风险，不能借下划线能力扩大 Markdown HTML 信任边界。
...
- Root 投票和逐消息已读必须分别受 `COMMUNITY_CHAT_POLLS_ENABLED`、`COMMUNITY_CHAT_READ_RECEIPTS_ENABLED` 显式子开关控制，并继续依赖消息总开关；关开关只停止新建/投票/记录，不删除历史投票或回执。投票问题属于消息正文，选择方式、每人上限、选项和截止时间属于投票子表。旧单选票继续保存在 `(message_id, user_id)` 唯一表；多选必须接收用户最新的完整选择集合，在同一事务内先删除旧明细、再批量写入新明细，禁止逐项请求形成中间态。多选上限由投票主表作为唯一事实源，服务端必须拒绝重复选项、越界数量和不属于该投票的选项；总参与人数按 `user_id` 去重，不能把各选项票数求和。创建仍复用消息事务、全负载幂等、提及和通知链路，但既有文本/表情消息指纹版本不得随新消息类型整体升级，旧单选保持 v3，多选字段单独进入 v4，避免滚动发布时同一 `clientRequestId` 被误判冲突。同一投票草稿的网络重试必须复用客户端请求 ID，服务端必须先识别既有投票再校验相对当前时间的截止窗口，不能因时间流逝拒绝已经成功的幂等重放。截止时间输入必须带 `Z` 或数值偏移并转换为 UTC，SQL 判定只使用 `UTC_TIMESTAMP(3)`；禁止把会话时区下的 `DATETIME` 与带 `_utc` 语义的列直接比较。进行中聚合结果只能在 Root 或当前成员已经投票时下发；尚未投票的普通成员必须移除 `totalVoterCount` 与各选项 `voteCount`，结束后才对所有读者公开。投票成员身份只能由真实 Root 主动选择某个选项后通过独立分页端点读取，Handler 与 Service 双层失败关闭，按权威 `selection_mode` 查询单选或多选事实表，未知模式不得猜测；响应只含社区公开身份，禁止把名单塞入消息历史或聚合结果。逐消息回执在发送 Root 消息时固化全局子开关，只记录登录用户对当前可见 active 消息的首次阅读，使用 `(message_id, user_id)` 幂等保存首次可见时间；作者本人、游客、个人删除、屏蔽和隐藏消息不计入。普通成员界面不得显示“会记录已读”或逐消息徽标，但不能据此停止静默采集。普通消息响应只给 Root 聚合数，Root 前台每 8 秒只能通过不含正文和成员身份的轻量批量端点校准当前窗口，成员名单必须由 Root 主动点击后通过独立分页接口按需读取；两类端点均需 Handler 与 Service 双层失败关闭，禁止返回内部账号 ID，也禁止把名单塞入每页消息历史。功能暂停后仍可读取既有数量和名单；8 秒被动数量端点不写通用 API 日志，真正写回执和治理操作仍按领域规则记录。气泡正文只有在页面可见、浏览器窗口聚焦且前台连续可见至少 800ms 后，才进入每批最多 30 条的写入队列；滚动、布局变化、失焦、切后台和恢复前台都必须重置连续计时，弱网失败使用静默退避重试，超过一批的同屏回执必须继续排空；房间 `community_chat_reads` 仍只表示未读游标，禁止拿它推导逐条阅读。回执资格查询禁止用互斥锁串行化消息行或 Root 账号，但必须用 MySQL 5.7 共享锁与账号注销建立先后顺序；已读回执不得广播实时事件，投票事件仅定向投票作者，前端对密集事件合并在途刷新并保证结束后补一次。
- 投票首次写入必须让数据库同时完成截止时间解析和 5 分钟～30 天窗口判断，解析为 `NULL` 时失败关闭；列表返回的 `serverTime` 也取自数据库 UTC 时钟，不能用应用进程时钟驱动截止 UI。客户端跨过截止点只立即请求一次详情，失败后交给既有安全轮询，禁止由每秒展示时钟放大故障。账号注销必须在角色去标识化前同步删除本人票据/回执；Root 消息创建、投票和回执资格查询都要与账号行锁建立确定顺序，异步清理只能作为幂等兜底。
