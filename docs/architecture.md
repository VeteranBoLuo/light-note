...
Root 可以把一条消息创建为单选投票：问题继续保存为消息正文，选项、UTC 截止时间、关闭状态和每账号一票的当前选择分别保存到投票子表，普通登录用户可在开放期投票或改票，游客只读。Root 始终可见聚合结果并可提前结束；普通用户只在结束后看到票数，不返回投票人名单。截止时间必须由客户端携带明确时区并在服务端转换为 UTC，允许范围为 5 分钟至 30 天；首次落库的相对时间窗口与后续截止判断都以事务内数据库 `UTC_TIMESTAMP(3)` 为准，投票、消息、提及、通知和幂等指纹共用同一事务，投票不能混入图片。逐消息已读子开关开启时，新发送的 Root 普通消息和投票自动固化回执标记；登录用户仅在页面可见、浏览器聚焦且消息气泡连续可见至少 800ms 后写入一次不可变的首次阅读事实，Root 只看到去重后的聚合人数，不看到名单，作者本人、游客、已删除/屏蔽/不可见消息不计入。关闭子开关后既有聚合数继续可查但不再新增。房间阅读位置继续只服务未读角标，不能代替逐消息回执；账号注销会在去标识化前同步移除本人票据与回执，并关闭其历史 Root 发言采集位。

> 当前项目在线人数由 PM2 单实例内的实时 Hub 聚合；若未来切换为 cluster 或多机部署，必须先把在线身份集合迁移到 Redis 等共享 presence 存储，避免各实例只返回自己的局部人数。

### 社区客厅访问与图文消息基础层

- API 挂载在 `/api/community-chat`，当前提供访问/申请/规则/房间、消息历史/发送/房间阅读位置/点赞/撤回、投票详情/投票/结束、逐消息已读回执批量写入、待发送图片上传/丢弃/鉴权读取、按消息公有 ID 读取作者公开资料与全部公开成就、当前账号名片读取与编辑、通知偏好，以及举报和屏蔽作者、屏蔽名单与解除屏蔽接口；Root 另有准入申请、成员撤销、举报列表与处置接口。消息历史查询的 `before` 与 `focus` 互斥：前者向上翻页，后者只为当前用户可见的 active/recalled 消息深链返回截至目标的历史窗口并标记是否还有更新消息。投票详情和已读人数始终按当前查看者重新鉴权、过滤并聚合，客户端不得提交结果、作者或目标账号。公共接口当前只接受 `general`，旧的 `announcements/newcomers/tips/co-build/topics/lounge` 数据通过兼容 migration 合并后归档，不能靠直连旧 slug 绕过前台。两类后台页面只消费这些受控 API，禁止绕过业务接口直接操作数据库。所有接口均不得扩张或复用私人 AI `/api/chat`。
...
- WebSocket 只广播 `message.created`、`message.updated`、`message.removed`、`runtime.changed`、定向 `access.changed` 等小型变化事件；点赞、撤回和投票状态变化共用 `message.updated`，事件只携带变化原因、房间和消息公有 ID，消息正文、投票结果、点赞名单、头像 Data URL 和账号身份不进入事件包。普通成员投票只定向通知投票作者 Root，提前结束才广播给房间；已读回执不产生实时事件，由 Root 端合并刷新与 30 秒安全刷新读取聚合数，避免把每次阅读放大成全房间广播。历史、屏蔽过滤、`isOwn` 与消息顺序仍以 REST/MySQL 为准。业务事务提交后才向当前进程和 Redis `community-chat:realtime:v1` 发布，Redis 发布/订阅失败不得回滚已提交消息。前端按 `eventId` 去重，1/2/4 秒指数退避并加抖动，重连、切网或回前台后立即调 REST 补齐；旧 3000 端口 echo 演示已移除。
...
- Schema 三轨同步为 `migrations/20260809_community_chat_foundation.sql`、`migrations/20260809_community_chat_text_mvp.sql`、`migrations/20260809_community_chat_governance.sql`、单房间数据兼容迁移 `20260809_community_chat_single_room.sql`、历史账号列排序规则修正 `20260809_community_chat_account_id_collation.sql`、运行策略迁移 `20260809_community_chat_runtime_policy.sql`、消息互动迁移 `20260810_community_chat_message_interactions.sql`、社区名片迁移 `20260811_community_chat_member_profiles.sql`、公开身份/稳定提及/自定义表情迁移 `20260813_community_chat_identity_mentions_stickers.sql`、投票/逐消息已读迁移 `migrations/20260826_community_chat_polls_read_receipts.sql`、`util/communityChatSchema.js` 和 `tag_db.sql`，只读门禁在 `migrations/schema-assertions.sql` 第 31/32/33/34/41/43/47/60 组；迁移和功能开关分离，建表本身不会开放社区或消息。生产发布先执行正常注册账号身份回填脚本的 dry-run，再显式 `--apply`，不能在 GET 搜索或历史读取中隐式批量建身份。
