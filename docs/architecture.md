...
- 派生预览以受控 `source_type` 区分个人云文件、聊天室临时文件和聊天室照片；来源类型参与唯一键及 OBS 派生对象键，各数据域不得只靠数值 ID 关联。聊天室来源仍先经过父消息可见性校验，现有云空间与分享预览接口保持独立。
- 文档解析、OCR、压缩包清单、Office 转换与图片派生由独立 Worker 处理；HTTP 进程只创建任务与读取状态。
- 派生资源有尺寸、条目数、超时、输出大小和进程并发上限；解析子进程不继承数据库、Redis 或 OBS 凭据。
- 云空间与聊天室照片完整保存原件，列表只读缩略图，打开预览再准备清晰图；查看原图是显式动作，缩放不触发原件下载，下载始终使用原件。动画派生仅展示首帧并提示原图播放；无有效优化结果时保留占位与原图入口。笔记正文新插图由 `prepareNoteImage.ts` 封装既有图片工具，按编辑会话选择保守优化或原图；旧正文、模板内嵌图、表情与贴纸不参与。
- 图片处理参数集中在 `util/filePreview/image.js`，缓存绑定来源、资源 ID、对象键、ETag、大小、规格和策略版本。生成发布前再次校验源身份及存活状态。原件直显模式不写派生对象键，清理器只能删除派生对象字段，替换失败必须保留可重试的对象引用。
- 派生图的批量准备和查询逐项复用来源权限；聊天只暴露照片 `publicId`，待发送照片仅上传者可读。分享首次预览遵循既有次数规则，后续查询、重试与查看原图复用票据，独立下载另行计次；管理员只读上下文不创建任务。
- 云空间派生缓存按最近使用时间回收；聊天缓存期限不得超过[聊天附件生命周期](#社区聊天室)，生成和读取都不续期，签名不得跨越源文件或分享期限。派生图不创建云文件、不占用户额度，成本以派生产物大小单独统计。三个业务入口可独立关闭；关闭生成不停止清理，关闭笔记优化不能还原已经压缩的正文图片。
...
- 文档/文件预览 Worker：解析、OCR、压缩包与 Office 与图片派生，并按来源类型隔离、回收个人云文件和聊天室附件的派生物。
- 知识工坊 Worker：租约领取积分任务，复用 AI Skill 或文档来源生成成果并结算积分。
- 整理建议 Worker：新旧队列共享文档进程；规则按记录数与序列化字节上限批量持久化，AI 每次调度处理一个资源。规则读取不放入持有用户锁的事务，持久化时以任务租约校验防止迟到写入。新任务的标签与标题共享一次结构化元信息调用，规则不经过 Provider；计量边界见 [AI Skills](./ai-skills.md)。外发持有与账号注销相同的用户行锁，结果在 Execution 成功前提交，并以任务与资源租约令牌防止迟到回写。过期的已运行任务无法证明 Provider 未执行，因此只标记中断，不自动再次收费；用户可明确重新整理。注销同步清理任务、快照和建议。
