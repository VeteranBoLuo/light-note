...
- 每日回顾是独立的账号日历业务域：`daily_review_sessions` 固定账号当天的会话，`daily_review_items` 固定最多三条书签、笔记或文件、候选判定时的账号本地资源日期及逐条动作；GET 只读已有会话，幂等 ensure 才生成清单。展示日期和回顾理由使用该固定日期，不按浏览器时区重新解释。打开内容或进入原因标签算作当日已回顾，全部有效条目处理后完成；用户也可以只收起今天，不产生任务、积分或连续天数压力。
