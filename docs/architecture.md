...
Root 可以把一条消息创建为单选或多选投票：问题继续保存为消息正文，选择方式、每人上限、选项、UTC 截止时间和关闭状态保存到投票子表。单选继续使用 `(message_id, user_id)` 唯一票表，保留旧实例与旧投票语义；多选使用独立选择明细表，并在同一事务内删除旧集合、写入最新完整集合。房间访问响应通过 `pollSelectionModes` 声明服务端能力；字段缺失的旧服务端只允许新客户端发布单选，避免多选草稿被静默降级。普通登录用户可在开放期投票或改票，游客只读。Root 始终可见聚合结果并可提前结束；普通用户只在结束后看到票数，不返回投票人名单。多选的总参与人数按账号去重，选项比例以参与人数为分母，因此比例合计可以超过 100%。截止时间必须由客户端携带明确时区并在服务端转换为 UTC，允许范围为 5 分钟至 30 天；首次落库的相对时间窗口与后续截止判断都以事务内数据库 `UTC_TIMESTAMP(3)` 为准。投票、消息、提及、通知和幂等指纹共用同一事务，旧单选投票保持 v3 指纹，多选新增字段使用 v4，投票不能混入图片。逐消息已读子开关开启时，新发送的 Root 普通消息和投票自动固化回执标记；登录用户仅在页面可见、浏览器聚焦且消息气泡连续可见至少 800ms 后写入一次不可变的首次阅读事实，作者本人、游客、已删除/屏蔽/不可见消息不计入。普通消息与历史响应仍只给 Root 去重聚合人数；Root 点击单条“已读 N 人”后，才通过独立分页接口按需读取成员昵称、社区公有 ID、头像/头像框和首次已读时间，普通成员响应永远不携带名单。关闭子开关后既有聚合数和名单继续可查但不再新增。房间阅读位置继续只服务未读角标，不能代替逐消息回执；账号注销会在去标识化前同步移除本人单选票、多选明细与回执，并关闭其历史 Root 发言采集位。
