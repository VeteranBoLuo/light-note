...
- 单个已选书签且用户明确要求“生成笔记”时，请求在通用 Semantic Planner 之前进入确定性专用链路：服务端先使用权威书签快照，快照不足再仅读取一次当前 URL，然后用强制 `submit_bookmark_note_draft` 协议生成完整 Markdown 草稿；格式不完整、有较长资料却过度简略，或“写长一点”后没有真正扩写时只允许一次同权限修复。该链路仅准备 `create_note` 确认卡，未确认前不写入笔记
