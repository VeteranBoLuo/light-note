...
子树删除使用 `tree_delete_batch_id` 记录一次删除批次：有后代的页面必须按实际子树数量确认并原子软删除；恢复只恢复同批节点，更早单独删除的页面不能被带回。硬删除、账号注销、JSON 导入/导出和回收站批量操作都必须携带页面树关系；导入先对外部 ID 做 owner 内重映射，再校验循环与 8 层上限。父页面本身仍是可编辑正文，详情底部以紧凑导航条展示直接子页面；页面路径中的父页面直接打开对应正文，只有“我的知识库”根节点返回列表，详情页之间沿树连续切换时不再层层嵌套列表来源 URL。“页面树目录”和只读取当前正文 H1/H2/H3 的“正文大纲”是两套独立结构。
