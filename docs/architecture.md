...
- 桌面端“快速添加”支持 URL、Markdown 文本、文件和待办；移动端只保留书签、笔记和文件，待办统一从底部“待办”一级入口新建。资源创建与加入待整理在同一业务事务或确认链中完成。桌面端快速待办只填写标题、日期预设和优先级即可直接进入待办列表，“完善详情”再携带当前草稿打开完整编辑器；完整待办编辑器在桌面端使用右侧抽屉、移动端使用底部抽屉，避免长表单挤在居中弹框中
...
- 标签一级页面 `/manage/tagMg` 提供“标签列表 / 知识地图”二级视图；知识地图不占用一级导航
- 历史 `/graph` 地址仅作兼容入口，重定向到 `/manage/tagMg?view=map`；资源中心的知识地图按钮也进入该二级视图
