...
- `support_checkout_intents`、`support_account_links` 与 `support_orders` 分别承担下单证据、可撤回关联和交易事实。通用日志跳过 Webhook 原始载荷，不保存留言、收货人、电话、地址或 OAuth 昵称头像；账号注销删除凭证/关联并把订单与轻笺账号去关联，但保留去标识化交易对账事实
- 订单状态每 5 分钟只重试到期 pending，小批量处理；全量历史最多每 6 小时同步一次且单进程合并，避免支持页轮询第三方。排行榜与任何公开昵称仍须单独取得明确授权，并支持匿名、不参与和撤回；核心功能不形成付费门槛
