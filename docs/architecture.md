...
- PC 顶栏 Root「管理」只直达后台管理；桌面后台左侧导航最末提供独立「服务器管理」入口，移动端继续从个人中心进入，避免在窄屏后台菜单与个人中心重复展示。`/serverManagement` 仍在现有应用外壳内，由 Root 路由守卫保护。服务器管理内部使用自己的响应式模块壳：PC 左侧导航、移动端 `BTabs`，固定子路由为 `/overview`、`/services`、`/security`、`/storage`、`/events`。
...
- 页面职责保持唯一：运行概览只展示主机指标、运行事实与趋势；服务进程是固定服务状态、资源占用、受限日志和白名单安全操作的唯一入口；操作审计只展示服务器写动作的意图、结果与原因，不再复制服务状态或维护第二套日志读取状态。
