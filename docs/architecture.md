...
└── util/                   # 工具模块
    ...
    │   └── tagSpaceService.js # 标签空间聚合、分页与关联标签派生读模型
    └── security/          # 安全攻击检测
```
...
标签空间是既有标签能力的浏览层，不是新建的“专题”实体。每个 `tag` 本身就是一个空间，`resource_tag_relations` 仍是书签、笔记和文件归入空间的唯一事实源。不增加主题标签分类、`is_topic` 字段、第二套资源成员关系或新表。

- `/manage/tagMg` 默认展示标签空间索引；`/manage/tagMg?mode=manage` 保留创建、编辑、排序和删除等治理能力，旧路由与外部深链继续可用。
- `/tag/:id` 是空间详情，以标签 ID 精确读取书签、笔记、文件及由双方仍有效的共享资源派生的关联标签；同名标签不得被合并。
- `/api/bookmark/queryTagSpaces` 提供服务端分页、关键词、资源类型和排序读模型；`/api/bookmark/getTagSpace` 提供单空间摘要与关联标签；`/api/bookmark/queryTagSpaceResources` 把三类有效资源先 `UNION ALL` 合并，再按“最近更新 / 最近加入标签”统一排序和分页。所有读只计当前 subject owner 且未删除的资源，单页最多 50 条。
- 空间索引默认隐藏零资源标签，但标签管理仍完整显示它们；待办是行动对象，不进入标签空间。
- 详情使用标签 ID 精确限定空间边界，不全量拉取后本地筛选，也不复用“按类型分段”的全局搜索有序分页冒充跨类型时间线；关系图和 AI 是渐进展开的次级能力，不阻塞默认资源浏览。
- 游客不能进入私人标签空间；管理员代看使用 `resourceUser` 指向的 subject 读模型，readonly 界面不暴露新建、编辑和标签治理入口。
...
更新日志使用 `update_logs` 单表保存标题、发布日期、摘要、兼容摘要、标签、Markdown 正文及该条日志拥有的 OBS object key 集合。编辑器以 Markdown 为唯一正文输入，历史重点更新首次编辑时自动转换为 Markdown，`highlights` 仅作为工作台等旧读模型的自动生成兼容字段。公开正文统一经 `marked + DOMPurify` 渲染；图片存放在 `update-logs/{logId}/` 前缀，页面使用稳定站内地址，由后端为私有 OBS 对象生成短时下载签名。文件选择与剪贴板图片共用同一上传和光标插入链路；纯文字粘贴保留浏览器原生行为，异步上传期间正文变化后只能使用完成时的真实光标，不能套用过期偏移。保存正文时按 Markdown 实际引用收敛 `image_keys`，事务提交后清理被移除的对象；删除日志同样先提交业务事务再清理 OBS。旧 `config_json` 数据由幂等迁移导入，迁移前公开读取仍可回退旧格式。
...
子树删除使用 `tree_delete_batch_id` 记录一次删除批次：有后代的页面必须按实际子树数量确认并原子软删除；恢复只恢复同批节点，更早单独删除的页面不能被带回。硬删除、账号注销、JSON 导入/导出和回收站批量操作都必须携带页面树关系；导入先对外部 ID 做 owner 内重映射，再校验循环与 8 层上限。父页面本身仍是可编辑正文；父页面主点击不再根据正文是否为空推断意图，而由账号偏好 `preferences.noteParentOpenMode` 决定：缺失或 `children` 默认进入直属子页面目录，桌面端可设为 `preview`，此时无论正文是否为空都预览当前父页面。移动端没有独立只读预览面板，父页面主点击固定进入子页面目录。目录标题栏的“打开正文”和详情页面树仍是显式正文入口，不受主点击偏好影响。桌面详情通过左侧共享页面树浏览子页面，移动详情底部只保留 52px 紧凑子页面入口。详情页之间沿树连续切换时不再层层嵌套列表来源 URL。“页面”和只读取当前正文 H1/H2/H3 的“大纲”是两套独立结构。
...
手绘笔记使用同一 `note` / `note_versions` 表，通过 `note.type = drawing` 区分，不新增表或第三方画布依赖。`content` 保存 `@lightnote/shared/drawing-note` 定义的 scene JSON：当前 v4 固定为 1448 × 1448 方形页面，允许受控的笔画、文本、基础形状与闭合区域填色元素；笔画、形状和填色可附带仅作用于自身的圆形擦除采样，渲染时在独立透明层完成像素擦除后再按原场景顺序合成，因此小橡皮可在粗笔画或填色内形成小圆孔且不会擦伤其下方元素。填色以按 y 升序、同行不重叠的 `[y, xStart, xEnd)` 扫描线保存，而不是保存一次性 Canvas 像素或仅保存种子点；场景限制为 750KB、1000 个元素、800 条笔画、5 万组笔画坐标、2000 条擦除轨迹、5 万组擦除采样、200 个文本框、300 个形状、100 个填色和 2 万条填色扫描线。形状只包含直线、箭头、矩形、圆角矩形、椭圆、三角形和菱形，保存有符号宽高以保留线条方向，不支持旋转或任意多边形。v1 的 1024 × 1448 旧画纸与 v2/v3 方形画纸继续可读；新编辑器在内存中将 v1 所有 x 坐标平移 212，v2/v3 只升级版本且不移动内容，均不缩放、不裁剪，只在用户实际保存后写成 v4。创建、专用保存、备份导入、详情读取与版本恢复均在服务端重新规范化；正文保存只走 `/api/note/updateDrawingNote` 并携带 revision，通用 `/updateNote` 拒绝手绘正文或类型提交。客户端上报最高支持的 `drawingSceneVersion`：高版本可读低版本，低版本不会收到高版本正文。

手绘编辑器以独立异步 chunk 加载，原生 Canvas 2D 只在手绘详情挂载；编辑态使用无原生滚动条的相机视口，首次进入按可用宽度缩放并居中、最高 100%，宽屏保持原始比例的 100% 书写尺度，画纸高度超出时通过手形工具平移；鼠标中键和右键在任意工具下临时进入平移，滚轮纵向增量以指针位置为锚点连续缩放，原生横向增量或 Shift+滚轮修改水平相机偏移，以上导航手势均不切换工具、不修改 scene 或历史。`S` 打开或关闭当前工具/选区的颜色与尺寸面板；填充作为形状后的独立工具，电脑点击或移动端点按对应像素执行四邻域扫描线填色，并保持激活以支持连续上色；`Command/Ctrl + 鼠标左键` 在画笔、橡皮、文字或形状工具下继续提供相同的快捷填色。样式快捷键在手形、无选区选择、文本输入、输入法组合和活动手势时不得抢占无关按键或指针；选择与手形工具继续保留原有修饰键语义。区域触达画纸边缘时按未闭合拒绝。scene v4 让笔画、形状轮廓与填色复用元素专属擦除遮罩：元素先在隔离透明层应用 `destination-out` 再按 z-order 合成，保证局部擦除不穿透下层；文本保持对象语义并由选择工具删除。文字工具提交后清空选择态，选择虚线只在选择工具显示；多选完成后使用统一组合包围框，命中框内空白也可拖动全部已选元素。直接预览仍由外层阅读区承载完整方形画纸，画板适配完成后只在首次进入时把画纸中心无动画对齐到视口中心，后续滚动和容器变化不得反复抢回位置。笔画采样与元素拖动使用非响应式临时对象并按 `requestAnimationFrame` 重绘，完成一次操作后才更新 Vue 状态，自动保存合并窗口为 3 秒。通用列表、全局搜索、标签图谱、AI、知识索引和目录分析不读取或解析 scene，手绘只按标题检索；分页列表始终省略 scene。手绘正文保存后，客户端从完整 scene 生成固定 480 × 270 WebP，并按用户、笔记、正文 revision 与共享渲染器版本上传派生缩略图；卡片 URL 同时包含 revision 和渲染器版本，保证渲染算法升级后旧的长期缓存位图自动失效。卡片接近可视区后优先读取该长期缓存位图，缺失时才通过 `/api/note/queryDrawingPreviews` 最多合并读取 12 篇当前用户的受限 scene 并在前端 Canvas 降级渲染。打开手绘详情时会对当前派生图发送轻量 HEAD 检查，历史派生图缺失则用已经加载的完整 scene 静默补齐，不改正文和 revision，也不要求用户手动保存。服务端降级预览限制到 120 个元素、1600 组轨迹点、1000 条填色扫描线和 4000 个文本字符；取景缩放下限为完整画纸 `contain`、上限为其 3 倍。离屏卡片不请求、不绘制。历史列表只回元素数，选定版本恢复时才读取完整内容。不支持图片元素、OCR、AI 正文编辑、模板或 HTML/Markdown/PDF 转换；单篇可导出 PNG 或原始 JSON，批量“原格式”导出使用 JSON。
...
笔记标题、正文或正文类型更新使用 `note.revision` 乐观并发控制；当前 Web 客户端写入必须携带已读取的 revision，服务端事务内 `FOR UPDATE` 后不匹配即返回 `NOTE_VERSION_CONFLICT` 和经过净化的当前快照，不允许静默覆盖。未保存草稿按账号/代管上下文与笔记 ID 隔离保存在 IndexedDB `lightnote-note-drafts-v1/noteDrafts`，250ms 合并写入、30 天过期且每个身份域最多 20 条；仅在 IndexedDB 不可用时使用 localStorage 应急副本，并在恢复后自动迁回。版本冲突只能由用户选择保留云端、另存副本或二次确认覆盖。`note_versions.source_revision` 记录快照对应版本，`reason` 区分自动保存、手动保存版本、格式转换、AI 修改、AI 撤销和恢复；`Command/Ctrl+S` 只立即执行普通保存，顶部“保存版本”按钮才强制写入 `reason = manual` 的还原点。HTML/Markdown 切换使用 `POST /api/note/convertMode`，请求必须携带 `baseRevision`、目标正文和预览指纹，服务端在同一事务内复核指纹、强制留档、更新正文/类型并同步资源引用。格式转换、AI 写入/撤销与版本恢复必须强制留档，不受普通自动保存的三分钟合并窗口影响。HTML 正文在创建、更新、AI 写入、导入、模板保存、新用户示例、历史恢复和旧数据读取边界统一经过服务端 allowlist 净化；Markdown 源码不得经过 HTML 净化器改写。

笔记详情顶栏按可用宽度分层：桌面端直接展示标签、历史版本和导出三个高频操作，其余低频动作保留在“更多”；平板和移动端继续从菜单或操作抽屉进入，不能因桌面直出而丢失入口。只读状态仍必须隐藏写操作，但保留导出等允许的只读能力。

桌面端从笔记库的卡片、列表或页面树打开已有叶子笔记时默认先进入库内只读预览，并保留明确的“编辑”按钮；设置中心可开启账号级 `preferences.noteDirectEdit`，开启后叶子笔记直接进入正式编辑页。父页面先由 `preferences.noteParentOpenMode` 裁决进入子页面目录或预览自身，优先级高于 `noteDirectEdit`，避免两个设置互相覆盖。两项设置只在 PC 显示；移动端叶子笔记始终直接进入编辑页，父页面始终进入子页面目录。新建笔记、目录“打开正文”和预览页内的“编辑”都是明确动作，不读取父页面主点击偏好。
...
云空间文件夹使用 `folders.parent_id` 组成同 owner、未删除的目录树，`NULL` 表示第一层，最大深度为 8；界面不渲染虚构根节点，而是在目录树上方固定显示聚合入口“全部文件”。顶部“新建文件夹”默认创建第一层，文件夹菜单可以新建子级、移动整棵子树、重命名和删除整棵子目录；同一父级内名称不可重复，不同分支允许同名。移动必须在账号行锁下用完整树快照校验归属、循环、同层重名和整棵子树的新深度；PC 无论是否跨层，拖到目标行上下边缘都表示插入目标同级的前后位置，拖到行中部表示移入目标文件夹，“全部文件”是把内部文件夹移到第一层末尾的专用拖放出口。三种目标都显示与笔记目录一致的实时文字提示；跨父级前后插入由移动接口在同一事务中同时更新父级与目标兄弟顺序，禁止先移动再调用第二个排序请求。菜单中的显式移动继续作为键盘与触控入口。新客户端必须在危险确认中明确说明“当前文件夹及所有子文件夹会被删除，文件会移回全部文件且不会被删除”，再显式提交递归删除意图；服务端在账号行锁事务中解除整棵子目录的文件归属后删除目录，旧客户端未提交该意图时仍只允许删空目录。文件夹菜单另提供“删除目录内全部文件”：服务端在同一目录树事务中锁定当前目录及所有后代的有效文件，复用单文件软删除链路将其移入回收站、移除待整理关系并撤销分享；默认保留目录归属和文件夹树，使恢复仍回到原目录。只有用户显式勾选“同时删除当前文件夹及所有子文件夹”时，事务才会先清空这些文件（含回收站记录）的 `folder_id` 再删除目录树，因此恢复后回到“全部文件”。旧客户端未声明 `treeVersion: 2` 时只返回第一层，避免把子级误画成同级后错误重排。书签仍使用多标签分类，不与云空间目录模型合并。
...
云空间批量下载在多选时必须先让用户选择“分别下载”或“打包为 ZIP”；单文件仍走普通下载。普通浏览器的“分别下载”不再申请本地目录权限：用户点击后直接逐项获取下载地址并交给浏览器，由浏览器保存到默认下载目录；浏览器若询问是否允许该站点下载多个文件，沿用原生权限交互。网页只能准确统计地址准备或提交阶段的失败，`anchor.click()` 只算“已提交给浏览器”，不得文案冒充“已保存”；取消只停止尚未提交的后续项。ZIP 模式继续用有进度、可取消的前端打包；Android App 因系统 `DownloadManager` 无法保存 WebView Blob，只提供逐个交接并在选择器中明确说明 ZIP 不可用，不能静默把所选文件改成另一种结果；新版 App 还需按消息 token 回传 `DownloadManager.enqueue` 结果，旧版无回执时禁止重发造成重复下载，界面按“已开始下载”中性收口并由后续原生进度表达真实完成状态。
...
Root 可以把一条消息创建为单选或多选投票：问题继续保存为消息正文，选择方式、每人上限、选项、UTC 截止时间和关闭状态保存到投票子表。单选继续使用 `(message_id, user_id)` 唯一票表，保留旧实例与旧投票语义；多选使用独立选择明细表，并在同一事务内删除旧集合、写入最新完整集合。房间访问响应通过 `pollSelectionModes` 声明服务端能力；字段缺失的旧服务端只允许新客户端发布单选，避免多选草稿被静默降级。普通登录用户可在开放期投票或改票，游客只读。Root 始终可见聚合结果并可提前结束；普通登录用户在完成自己的首次选择后即可看到聚合票数，尚未投票的成员仍看不到进行中结果，投票结束后所有读者可见聚合。普通用户永远不返回投票人身份；Root 只有主动点击“投票明细”并选择某一选项后，才通过独立分页接口读取该选项的公开昵称、社区 ID、头像与头像框，成员身份不进入消息历史或聚合结果。多选的总参与人数按账号去重，选项比例以参与人数为分母，因此比例合计可以超过 100%。截止时间必须由客户端携带明确时区并在服务端转换为 UTC，允许范围为 5 分钟至 30 天；首次落库的相对时间窗口与后续截止判断都以事务内数据库 `UTC_TIMESTAMP(3)` 为准。投票、消息、提及、通知和幂等指纹共用同一事务，旧单选投票保持 v3 指纹，多选新增字段使用 v4，投票不能混入图片。逐消息已读子开关开启时，新发送的 Root 普通消息和投票自动固化回执标记；登录用户仅在页面可见、浏览器聚焦且消息气泡连续可见至少 800ms 后写入一次不可变的首次阅读事实，作者本人、游客、已删除/屏蔽/不可见消息不计入。普通成员界面不显示逐消息已读徽标，但静默记录规则不受展示权限影响；普通消息与历史响应仍只给 Root 去重聚合人数。Root 点击单条“已读 N 人”后，才通过独立分页接口按需读取成员昵称、社区公有 ID、头像/头像框和首次已读时间，普通成员响应永远不携带名单。关闭子开关后既有聚合数和名单继续可查但不再新增。房间阅读位置继续只服务未读角标，不能代替逐消息回执；账号注销会在去标识化前同步移除本人单选票、多选明细与回执，并关闭其历史 Root 发言采集位。

> 当前项目在线人数由 PM2 单实例内的实时 Hub 聚合；若未来切换为 cluster 或多机部署，必须先把在线身份集合迁移到 Redis 等共享 presence 存储，避免各实例只返回自己的局部人数。

### 社区客厅访问与图文消息基础层

- API 挂载在 `/api/community-chat`，当前提供访问/申请/规则/房间、消息历史/发送/房间阅读位置/点赞/撤回、投票详情/投票/结束、逐消息已读回执批量写入、Root 批量校准当前窗口聚合数及按单条消息分页读取已读成员、待发送图片上传/丢弃/鉴权读取、按消息公有 ID 读取作者公开资料与全部公开成就、当前账号名片读取与编辑、通知偏好，以及举报和屏蔽作者、屏蔽名单与解除屏蔽接口；Root 另有准入申请、成员撤销、举报列表与处置接口。消息历史查询的 `before` 与 `focus` 互斥：前者向上翻页，后者只为当前用户可见的 active/recalled 消息深链返回截至目标的历史窗口并标记是否还有更新消息。投票详情和已读人数始终按当前查看者重新鉴权、过滤并聚合，客户端不得提交结果、作者或目标账号；已读数量与名单端点都在 Handler 与 Service 双层校验真实 Root，拒绝管理员预览身份，批量数量响应不含正文或成员身份，名单响应不返回内部账号 ID。公共接口当前只接受 `general`，旧的 `announcements/newcomers/tips/co-build/topics/lounge` 数据通过兼容 migration 合并后归档，不能靠直连旧 slug 绕过前台。两类后台页面只消费这些受控 API，禁止绕过业务接口直接操作数据库。所有接口均不得扩张或复用私人 AI `/api/chat`。
...
- `COMMUNITY_CHAT_ACCESS_MODE` 接受 `closed`（默认）、`public` 和为未来私密频道保留的 `invite_only`；未知值回退 `closed`。`public` 也必须同时显式设置 `COMMUNITY_CHAT_MESSAGING_ENABLED=true`，realtime 继续依赖消息总开关；投票与逐消息已读再分别要求显式开启 `COMMUNITY_CHAT_POLLS_ENABLED=true`、`COMMUNITY_CHAT_READ_RECEIPTS_ENABLED=true`，任一子开关都不能绕过消息总开关；候补名单与规则版本只服务邀请制流程。
...
- WebSocket 只广播 `message.created`、`message.updated`、`message.removed`、`runtime.changed`、定向 `access.changed` 等小型变化事件；点赞、撤回和投票状态变化共用 `message.updated`，事件只携带变化原因、房间和消息公有 ID，消息正文、投票结果、点赞名单、头像 Data URL 和账号身份不进入事件包。普通成员投票只定向通知投票作者 Root，提前结束才广播给房间；已读回执不产生实时事件，Root 停留在聊天室前台时每 8 秒通过轻量批量端点校准当前窗口的聚合数，点击徽标再立即按需读取单条名单，避免把每次阅读放大成全房间广播或每 8 秒重载整页消息。历史、屏蔽过滤、`isOwn` 与消息顺序仍以 REST/MySQL 为准。业务事务提交后才向当前进程和 Redis `community-chat:realtime:v1` 发布，Redis 发布/订阅失败不得回滚已提交消息。前端按 `eventId` 去重，1/2/4 秒指数退避并加抖动，重连、切网或回前台后立即调 REST 补齐；旧 3000 端口 echo 演示已移除。
...
- Schema 三轨同步为 `migrations/20260809_community_chat_foundation.sql`、`migrations/20260809_community_chat_text_mvp.sql`、`migrations/20260809_community_chat_governance.sql`、单房间数据兼容迁移 `20260809_community_chat_single_room.sql`、历史账号列排序规则修正 `20260809_community_chat_account_id_collation.sql`、运行策略迁移 `20260809_community_chat_runtime_policy.sql`、消息互动迁移 `20260810_community_chat_message_interactions.sql`、社区名片迁移 `20260811_community_chat_member_profiles.sql`、公开身份/稳定提及/自定义表情迁移 `20260813_community_chat_identity_mentions_stickers.sql`、投票/逐消息已读迁移 `migrations/20260826_community_chat_polls_read_receipts.sql`、多选投票迁移 `migrations/20260827_community_chat_multiple_choice_polls.sql`、`util/communityChatSchema.js` 和 `tag_db.sql`，只读门禁在 `migrations/schema-assertions.sql` 第 31/32/33/34/41/43/47/60 组；迁移和功能开关分离，建表本身不会开放社区或消息。生产发布先执行正常注册账号身份回填脚本的 dry-run，再显式 `--apply`，不能在 GET 搜索或历史读取中隐式批量建身份。
...
- 工具栏点击只通过浏览器原生行为打开 Side Panel，不读取或保存标签页信息。入口页先让用户选择书签、笔记或文件：进入书签视图后才用 `tabs` 读取当前活动标签页的 URL/标题，并凭 `activeTab + scripting` 尽力读取当前选中文本；选区注入失败不能阻断 URL/标题，网址右侧按钮只读取标签页元数据。笔记视图进入时只在内存准备当前 URL 的精确 Origin，不读 DOM；只有点击“带入当前网页文字”才通过可选 Host 权限按当前网站授权并读取顶层页面最多 5 万字可见文字，标签页变化时失败关闭。文件视图不读取网页。没有常驻内容脚本、`cookies` 或必需的 `<all_urls>`，不得遍历标签页、调用历史 API或持久化浏览记录。
...
- `/manage/tagMg` 默认是标签空间浏览层；`/manage/tagMg?mode=manage` 才是标签新增、编辑、排序和删除的治理层。资源中心的“全部资源 / 待整理”只表达资源范围，知识地图不作为第三个范围页签
...
- 邮箱注册和 GitHub 首次建号会按注册语言初始化 4 个带 Base64 SVG 图标的标签、3 个书签、5 篇笔记（包含标题固定为“手绘笔记示例”的手绘样例；其画布内容来自版本化静态快照，不在注册时读取来源账号）、1 个示例文件夹和 2 份云文件；已有账号登录、GitHub 绑定已有邮箱账号和历史用户都不会补发或重复生成。
- 共享游客账号不经过注册 Seed；其固定手绘示例通过 `pnpm --filter server sync:visitor-drawing-sample` 只读预检，并在获得发布或线上数据同步授权后追加 `-- --apply` 原位同步。该脚本与新账号 Seed 共用同一份静态场景，只接受固定笔记 ID、有效游客归属和 `drawing` 类型，写入前保存旧版本并递增 revision，不新增第二篇示例。
...
- 段位特权（`util/growth.js` 的 `RANKS`）被多个模块读取：**AI 每日 token 额度**（`aiQuota`）、**回收站保留天数**（`trashHandle`）、**云空间容量**、**每日抽奖次数**。所有登录角色均以 `user_growth.exp -> levelForExp()` 为唯一等级事实源，禁止再在资料卡、社区、商店或抽奖中按 `root` 伪造满级。`20260825_ai_bonus_wallet_and_afdian_rewards.sql` 会把上线前已有 root 的历史虚拟 Lv.15 一次性固化为可审计的 50000 EXP/Lv.15，以保留既有存储权益；新增或后续转为 root 的账号不会自动满级。AI 调用先消耗当日等级额度，耗尽后才扣 `user_growth.ai_bonus_tokens` 永久余额；兑换、抽奖与历史背包转换都只能增加此余额。改等级阈值、特权表或余额扣减顺序会牵连这些模块，不是孤立改动。
...
- PC 顶栏 Root「管理」只直达后台管理；桌面后台左侧导航最末提供独立「服务器管理」入口，移动端继续从个人中心进入，避免在窄屏后台菜单与个人中心重复展示。`/serverManagement` 仍在现有应用外壳内，由 Root 路由守卫保护。服务器管理内部使用自己的响应式模块壳：PC 左侧导航、移动端 `BTabs`，固定子路由为 `/overview`、`/diagnostics`、`/services`、`/security`、`/storage`、`/events`。
...
- 实时指标由 Agent 每 3 秒采样；服务快照在 Agent 内最多缓存 10 秒，安全慢检查最多缓存 5 分钟，存储页复用实时采样历史。分域 Socket/API 端点为 `/dashboard`、`/services`、`/storage`、`/security`；Express 额外提供 `/diagnostics`，并行聚合这些既有快照、允许单域失败，再根据后端固定阈值输出 `pass / warning / fail / unknown` 检查项。诊断不新增 Agent 命令，也不把 SSH 日志、APT 或防火墙等慢检查塞回 3 秒仪表盘。概览前台默认每 3 秒刷新，可切换为 10 秒、30 秒、1 分钟、5 分钟或暂停；服务页 10 秒、存储页 3 秒、审计事件页 30 秒，安全页与诊断页按需刷新。页面隐藏时停止请求，已有可用数据后的瞬时失败保留旧快照并明确提示。所有被动快照和诊断均不写通用 API 日志，运维写动作继续进入独立审计。
- 页面职责保持唯一：运行概览只展示主机指标、运行事实与趋势；诊断与处置把四域事实变成按优先级排序的处理清单，只导航到权威模块，不复制写按钮；服务进程是固定服务状态、资源占用、受限日志和白名单安全操作的唯一入口，并提供服务筛选、日志关键词/级别/自动刷新/导出以及操作后回执核验；操作审计只展示服务器写动作的意图、结果与原因并支持本地检索、汇总和导出，不再复制服务状态或维护第二套日志读取状态。
