...
- 七个写工具（含保存附件到云空间、创建图片笔记）使用 Redis 哈希键保存的一次性确认令牌；前端先展示风险、目标和影响范围，确认后按服务端保存参数执行
...
- 临时附件可经确认式写工具复制到永久云空间；图片附件可经确认式写工具写入本站图片目录、创建 HTML 图片笔记并同步登记 `note_images` 引用。两条链路都重新校验附件归属、有效期、文件类型和容量，不依赖 OCR 成功
