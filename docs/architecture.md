...
数据来自 `POST /api/workbench/today` 轻量聚合，不复用完整工作台接口（后者会额外跑趋势、饼图、排行和最近列表共 8 组查询）。明细按今日页展示上限截断（逾期 3、今天 4、待整理 3），但摘要计数使用权威总数，不能把「最多取 3 条」显示成「只有 3 条逾期」。移动端快速记录只覆盖书签、笔记、文件；待办按既有约定跳待办模块新建。
...
「默认首页」只在桌面端作为设置项出现：它影响登录后的落点和桌面 Logo（Logo 进 `/app`，桌面按该偏好解析）。移动端不显示这一项——Logo 固定回「今日」，登录落点也由移动端默认值（今日）决定，暴露一个不生效的开关只会误导。
...
- Provider 协议抖动采用“先隔离、再受限恢复”：计划外的已知只读调用会被丢弃且绝不执行，不会拖垮原计划；已验证读取计划只缺真实调用时，最多两次把目录收窄到缺失 read capability 让 AI 补齐，再与原完整计划重新求交，补全轮不能增加能力或把读取升级为写入；结构化计划缺失或自身冲突时，最多一次在原权限和完整能力目录内让 AI 重新判断语义，重判即使改变请求分类也仍须重新通过参数、权限、确认与回执校验。恢复失败时保持原安全裁决。正常请求不增加模型往返，只有异常计划进入恢复
...
- Final Reply 还经过供应商无关的质量门禁：有工具结果、引用证据或动作语义的事实回答会把最终温度限制在 `0.6` 以内，并先完整缓冲、校验后再一次性发给客户端，避免异常原文已经流出后无法撤回；`finish_reason=length/max_tokens/content_filter`、内部结束标记、超长无断句片段、工具协议泄漏或高比例重复会触发一次禁用工具、低温、收紧输出预算的重试；重试仍不合格时只返回稳定错误文案，不把截断或循环退化的原文作为权威终态。该门禁只判断可验证的生成退化信号，不替代工具事实、引用和执行回执校验
...
- `evaluation/ai-assistant/` 的 schema v2 提供 268 条完全合成任务、49 个合成来源和六维确定性评分器，覆盖 10 个能力域且每域至少 20 条；owner 四维、请求 lifecycle、反凑数校验和生成器 `--check` 都进入数据契约。它不加载 `.env`、不连接数据库/Redis/模型、也不访问网络；`.github/workflows/ci.yml` 已在测试后运行生成器一致性检查与 `eval:ai-assistant`，本地结构化数量和静态 CI 门槛已经达到。仍缺真实 Agent adapter/回放、自然语言有用性评测和人工引用蕴含标注。
...
- 桌面端“快速添加”支持 URL、Markdown 文本、文件和待办；移动端只保留书签、笔记和文件，待办统一从底部“待办”一级入口新建。资源创建与加入待整理在同一业务事务或确认链中完成，桌面端快速添加的待办则直接进入待办列表
...
- 移动端底部一级入口为“待办”（位于“今日”“资料”“AI”之后），默认只展示待办；待整理资源从“资源中心 → 待整理”进入，且不混入待办
