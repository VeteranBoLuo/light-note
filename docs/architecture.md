...
- Final Reply 还经过供应商无关的质量门禁：有工具结果、引用证据或动作语义的事实回答会把最终温度限制在 `0.6` 以内，并先完整缓冲、校验后再一次性发给客户端，避免异常原文已经流出后无法撤回；普通流式回答在生成过程中也持续检查长中文无断句、循环重复、协议泄漏等明确退化信号，命中后会主动取消上游流，而不是继续消耗到模型上限；`finish_reason=length/max_tokens/content_filter` 或上述退化会触发一次禁用工具、低温、收紧输出预算的重试，重试正文只通过权威终态整体替换异常前缀；重试仍不合格时只返回稳定错误文案。该门禁只判断可验证的生成退化信号，不替代工具事实、引用和执行回执校验
