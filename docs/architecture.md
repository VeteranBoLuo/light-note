...
- **没有自动迁移 runner**：`migrations/*.sql` 是手工执行的幂等脚本（自带 `information_schema` 判断）；部分（如 `rename_admin_to_user`、`conversion_events_ip`）注明由 DBA 手动跑。
- **大量表靠 app 启动时 `ensure*()` 运行时建表/补列**，既不在 migrations、也多不在上方核心表清单：积分系统的 `growth_events` / `user_growth`（及其 `points`/`equipped_*`/`storage_bonus_mb`/`lottery_*` 列）/ `points_log` / `user_cosmetics` / `user_item` / `ai_daily_bonus`、书签的 `bookmark_snapshot` / `bookmark_health`、`notification`（`batch_id`/`recalled` 列）、`feature_requests` 等。**只读 `migrations/` 或核心表清单会严重低估真实 schema。**
- **Schema 漂移**：`note.type`、整张 `note_versions`、`bookmark.is_top`、`files.share_token` 被代码大量使用，却无任何建表/加列 DDL（应为生产手工应用或基线 dump 过期）。**全新环境按仓库建库会缺列/缺表报错，以线上真实库为准。**
