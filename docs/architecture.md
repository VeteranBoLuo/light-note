...
资源中心 `/search/global` 按资源类型分别分页，每类每页最多 12 条，并返回经过关键词、资源类型、标签、无标签、日期和排序条件后的 `typeTotals`、`hasMoreByType` 与标签筛选选项。前端不再用 `limit=0` 拉取书签、笔记、文件和标签全量后本地筛选；滚动加载只追加当前查询快照的下一页，请求版本号负责阻止旧关键词或旧筛选响应覆盖新结果。
