...
- 移动端底部一级导航为“今日｜资料｜工具箱｜待办｜聊天室”，个人中心由顶栏头像进入；今日页顶栏加号继续承担快速添加。
...
- 浏览器本地工具、积分任务、AI Skill 与文档 Worker 共用版本化工具目录，但保持不同执行和数据边界；完整规范见 [知识工具箱](./toolbox.md)。
...
| 工具箱     | 持续工作区、资料引用、事项、推进记录，以及报价、任务、成果与保存收据    |
...
- 工具箱 Worker：租约领取积分任务，复用 AI Skill 或文档来源生成成果并结算积分。
