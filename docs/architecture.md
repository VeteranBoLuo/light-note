...
- 通知中心是浏览器推送的唯一来源；各业务保持原有通知范围。新老用户的浏览器通知偏好默认开启，之后主动关闭按账号保存在当前浏览器；旧账号级偏好不决定设备状态。已有系统授权时自动订阅，未授权时由用户点击授权按钮完成授权，拒绝或订阅失败不能展示为已接收。客户端持久化绑定后才激活服务端订阅；前台照常提醒，多标签页由同一 Service Worker 去重，Android 原生壳不接入。
