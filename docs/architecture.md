...
今日首屏同时承担统一总览与当天行动：顶部总览与桌面工作台保持同一口径，依次展示全部未完成待办、待整理和未读通知；移动端为避免三等分入口拥挤，第二项固定使用短名称「待整理」。日期问候中的件数仍只统计逾期、今天到期和待整理，表达今天应优先处理的事项；下方继续展示今日待办与待整理明细（复用 `TodayActionSection` 的完成、延期、编辑和整理能力），逾期状态在明细中用危险色明确标出。页面不放「查看全部待办」按钮——底部导航已有待办一级入口，重复入口只会占位。资源总量、增长趋势、文件类型分布、常用标签排行和最近更新只留在桌面工作台，不搬进移动首屏。

桌面工作台和移动「今日」在关键摘要之后共用 `DailyReviewCard` 展示轻量「每日回顾」：继续复用 `/growth/recap` 的账号日历、稳定日选、最近沉淀 / 那年今日 / 尘封内容和 `growth_recap_state` 屏蔽状态，不另建每日回顾表或第二套随机规则。卡片每次只展示当前分类的一条内容，可在分类内换一条，并复用 `/growth/recap/state` 的 7 天后提醒与永久停止推荐；成长页的 `RecapCard` 保留三组完整列表。每日回顾不并入 `POST /api/workbench/today`，而是在今日摘要稳定后独立渐进读取；首次失败显示局部重试，已有数据刷新失败继续保留旧内容，不阻断待办、快速记录和成长任务。多个入口共用 `useGrowth` 的账号隔离缓存与同账号在途请求，偏好写入后的强制回读会作废写入前的旧响应。
