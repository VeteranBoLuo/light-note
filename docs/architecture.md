...
- Agent 工具通过 `util/agent/sourceUtils.js` 统一生成、清洗和去重来源；来源必须携带稳定资源 ID 与显式语义目标，外链仅接受 HTTP(S)。当前语义目标覆盖笔记详情、书签原网页/编辑/快照、云文件精确预览、云文件夹、帮助文章、公开知识、Root 内部知识、标签详情、网页和临时文档。帮助中心、Root 知识库、云空间文件/文件夹和书签快照均把资源 ID 写入查询参数，刷新与浏览器前进后退可恢复同一内容
