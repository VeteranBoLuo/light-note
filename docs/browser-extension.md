...
用户安装、首次保存和故障处理见 [入门指南](./getting-started.md#在电脑浏览网页时随手收)。本文维护开发协议与权限边界。
...
- 书签草稿只在同一次侧栏打开期间保留，便于返回入口后继续编辑。真正关闭并重新打开侧栏时，旧页面的 URL、名称、描述、标签和幂等状态全部丢弃，再从新的当前活动页开始收集。
...
- 插件不打包 TinyMCE，使用无工具栏编辑器；主站笔记编辑器独立维护。
...
- 书签/笔记在首次请求前把“载荷 SHA-256 指纹 + 幂等键”写入草稿；响应丢失或会话过期后，相同载荷复用原键，载荷发生变化则换新键。笔记关闭侧栏后仍恢复未提交草稿；书签遵循上面的侧栏实例隔离规则，重新打开后从当前页创建新草稿和新操作状态。
