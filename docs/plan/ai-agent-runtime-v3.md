...
### 本分支确定性验收记录（2026-08-22）

- 服务端全量测试：360 个测试文件、3013 项测试全部通过。
...
- 隔离 Schema 行为门禁：在本机临时 MySQL 8.4（禁用 TCP、独立数据目录）中连续执行 `20260821_agent_runtime_state.sql` 两次均成功；五张表、关键索引、6 个外键和 revision 默认值符合合同。合成 Conversation/Run/SourceSet/ResultSet/ArtifactVersion 写入后，revision CAS 仅首次命中，删除 Conversation 后五实体均由外键回收。该验证不连接轻笺数据库，也不能替代发布时在目标 MySQL 上执行的只读 `check:schema`。
...
- 同步最新 `main` 后，前端全量测试为 426 个测试文件、2711 项通过；另有 2 项 Android 发布一致性检查因本地 APK 按约定不入库而按测试定义跳过。此前画布 schema v3/v4 的两项基线失败已随 `main` 的手绘升级修复。
