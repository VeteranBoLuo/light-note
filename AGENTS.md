...
- 用户明确说出"部署"、"deploy"、"上线"时，视为同时授权该部署流程必需的构建、打包和上传步骤，无需再单独确认构建
- 仅执行构建而不部署时，仍需用户明确说出"build"、"构建"、"打包"才算授权
...
- 提交、推送、部署、连接服务器、操作线上数据库均以用户当前任务中的明确指令为准，不能互相推导授权；但明确部署已包含该部署流程必需的构建、打包和上传授权。
