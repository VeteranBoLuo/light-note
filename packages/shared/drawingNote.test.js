...
describe('drawingNote scene protocol', () => {
  ...
  it('只保留受支持字段并规范坐标精度', () => {
    expect(
      parseDrawingScene({
        ...
        ignored: true,
        elements: [
          { id: 's_1', kind: 'stroke', color: '#00a884', width: 4, points: [1.234, 2.345], extra: 1 },
          { id: 't_1', kind: 'text', x: 10, y: 20, width: 180, fontSize: 28, color: '#1f2937', text: '文本' },
        ],
      }),
    ).toEqual({
      v: 1,
      page: { width: 1024, height: 1448 },
      elements: [
        { id: 's_1', kind: 'stroke', color: '#00a884', width: 4, points: [1.23, 2.35] },
        { id: 't_1', kind: 'text', x: 10, y: 20, width: 180, fontSize: 28, color: '#1f2937', text: '文本' },
      ],
    });
  });
  ...
  it.each([
    ['脚本色值', { id: 's', kind: 'stroke', color: 'url(javascript:1)', width: 2, points: [0, 0] }],
    ['奇数坐标', { id: 's', kind: 'stroke', color: '#1f2937', width: 2, points: [0, 0, 1] }],
    ['外链元素', { id: 'i', kind: 'image', url: 'https://example.com/a.png' }],
  ])('拒绝%s', (_label, element) => {
    expect(() => parseDrawingScene({ v: 1, page: { width: 1024, height: 1448 }, elements: [element] })).toThrow(
      DrawingSceneValidationError,
    );
  });
